...
namespace Nop.Services.Catalog
{
    ...
    {
        /// <summary>
        /// Gets an appropriate advanced price
        /// </summary>
        ...
        public static decimal? GetAppropriateAdvancedPrice(this Product product, Customer customer, int storeId, int quantity)
        {
            if (!product.HasAdvancedPricing)
                return null;

            //get actual advanced prices
            var actualAdvancedPrices = product.AdvancedPrices.OrderBy(price => price.Quantity).ToList()
                .FilterByStore(storeId)
                .FilterForCustomer(customer)
                .FilterByDate()
                .RemoveDuplicatedQuantities();

            //get the most suitable advanced price based on the passed quantity
            var advancedPrice = actualAdvancedPrices.LastOrDefault(price => quantity >= price.Quantity);

            return advancedPrice != null ? (decimal?)advancedPrice.Price : null;
        }
