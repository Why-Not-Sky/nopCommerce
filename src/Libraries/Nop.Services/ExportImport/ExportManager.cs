    ...
    public partial class ExportManager : IExportManager
    {
        ...
        public virtual string ExportProductsToXml(IList<Product> products)
        {
            ...
            foreach (var product in products)
            {
                ...
                if (!IgnoreExportPoductProperty(p => p.AdvancedPricing))
                {
                    xmlWriter.WriteStartElement("AdvancedPrices");
                    var advancedPrices = product.AdvancedPrices;
                    foreach (var advancedPrice in advancedPrices)
                    {
                        xmlWriter.WriteStartElement("AdvancedPrice");
                        xmlWriter.WriteString("AdvancedPriceId", advancedPrice.Id);
                        xmlWriter.WriteString("StoreId", advancedPrice.StoreId);
                        xmlWriter.WriteString("CustomerRoleId", advancedPrice.CustomerRoleId, defaulValue: "0");
                        xmlWriter.WriteString("Quantity", advancedPrice.Quantity);
                        xmlWriter.WriteString("Price", advancedPrice.Price);
                        xmlWriter.WriteString("StartDateTimeUtc", advancedPrice.StartDateTimeUtc);
                        xmlWriter.WriteString("EndDateTimeUtc", advancedPrice.EndDateTimeUtc);
                        xmlWriter.WriteEndElement();
                    }
                    xmlWriter.WriteEndElement();
                }
